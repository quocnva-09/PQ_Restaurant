
//Bento
import bento1 from './bento1.png'
import bento2 from './bento2.png'
import bento3 from './bento3.png'
import bento4 from './bento4.png'
import bento5 from './bento5.png'

//Drink
import coca_drink from './coca_drink.jpg'
import liptonchanh_drink from './liptonchanh_drink.png'
import pepsi_drink from './pepsi_drink.jpg'
import samlanh_drink from './samlanh_drink.png'
import sapporo_drink from './sapporo_drink.jpg'
import tiger_drink from './tiger_drink.jpg'
import tradao_drink from './tradao_drink.jpg'
import trada_drink from './trada_drink.png'
import trahoacuc_drink from './trahoacuc_drink.jpg'
import sodablueocean_drink from './sodablueocean_drink.png'
import sodachanh_drink from './sodachanh_drink.png'
import sodadao_drink from './sodadao_drink.png'
import sodadau_drink from './sodadau_drink.png'
import sodaruoumo_drink from './sodaruoumo_drink.png'
import sodavietquat_drink from './sodavietquat_drink.png'

//Udon Curry
import beef_curry_udon from './beef_curry_udon.jpg'
import chicken_curry_udon from './chicken_curry_udon.jpg'

//Rice
import butadon_rice from './butadon_rice.png'
import shishirian_raisu_rice from './shishirian_raisu_rice.png'

//Curry
import com_bo_curry from './com_bo_curry.jpg'
import com_ga_curry from './com_ga_curry.jpg'
import com_heo_curry from './com_heo_curry.jpg'
import com_tom_curry from './com_tom_curry.jpg'
import sausage_curry_udon from './sausage_curry_udon.jpg'
import shrimp_curry_udon from './shrimp_curry_udon.jpg'

//Icon
import icon_account from './icon_account.png'
import search from './search.svg'
import basket from './basket.svg'
import menu from './menu.svg'
import menu_close from './menu-close.svg'
import user from './user.svg'
import user1 from './user1.png'
import user2 from './user2.png'
import user3 from './user3.png'
import user4 from './user4.png'
import badge from './badge.svg'
import order_icon from './order_icon.png'
import cart_added from './cart-added.svg'
import cart_add from './cart-add.svg'
import cart_remove from './cart-remove.svg'
import dashboard from './dashboard.svg'
import delivery from './delivery.svg'
import forward from './forward.svg'
import graph from './graph.svg'
import heart_add from './heart-add.svg'
import list from './list.svg'
import map from './map.svg'
import minus from './minus.svg'
import plus from './plus.svg'
import pound from './pound.svg'
import return_request from './return-request.svg'
import secure from './secure.svg'
import phone from './phone.svg'
import square_plus from './square-plus.svg'
import trash from './trash.svg'
import twitter from './twitter.svg'
import star from './star.svg'
import star_black from './star-black.svg'

//Udon Hot
import ebi_wakame_udon_hot from './ebi_wakame_udon_hot.jpg'
import egg_curry_udon from './egg_curry_udon.jpg'
import gobouten_onsen_udon_hot from './gobouten_onsen_udon_hot.jpg'
import gobou_ebi_tempura_udon_hot from './gobou_ebi_tempura_udon-hot.jpg'
import kitsune_niku_udon_hot from './kitsune_niku_udon_hot.jpg'
import kitsune_onsen_udon_hot from './kitsune_onsen_udon_hot.jpg'
import niku_kamatama_udon_hot from './niku_kamatama_udon_hot.jpg'
import niku_onsen_udon_hot from './niku_onsen_udon_hot.jpg'
import niku_wakame_udon_hot from './niku_wakame_udon_hot.jpg'
import shifudo_kimchi_udon_hot from './shifudo_kimchi_udon_hot.jpg'
import shifudo_udon_hot from './shifudo_udon_hot.png'
import unagi_udon_hot from './unagi_udon_hot.jpg'
import yaki_kimchi_udon_hot from './yaki_kimchi_udon_hot.jpg'

//Logo
import logo from './logo.jpg'
import logo40x40 from './logo40x40.png'
import logo40x40_circle from './logo40x40_circle.png'
import bg from './udon_bg.png'

//Poster
import poster3 from './poster3.png'
import poster4 from './poster4.jpg'
import poster5 from './poster5.png'
import poster6 from './poster6.jpg'
import poster7 from './poster7.jpg'

//Promotion
import promotion6 from './promotion6.jpg'

//Salad
import salad_cahoi from './salad_cahoi.jpg'
import salad_dauhutrung from './salad_dauhutrung.jpg'
import salad_rau from './salad_rau.jpg'
import salad_rongtrung from './salad_rongtrung.jpg'
import salad_thittrung from './salad_thittrung.jpg'
import salad_thitxongkhoi from './salad_thitxongkhoi.jpg'
import salad_tom from './salad_tom.jpg'

//Feature
import feature1 from './features1.png'
import feature2 from './features2.png'
import feature3 from './features3.png'

//Blog
import blog1 from './blog1.jpg'
import blog2 from './blog2.jpg'
import blog3 from './blog3.jpg'
import blog4 from './blog4.jpg'
import blog5 from './blog5.jpg'
import blog6 from './blog6.jpg'
import blog7 from './blog7.jpg'
import blog8 from './blog8.jpg'

import mail from './mail.svg'
import right from './right.svg'
import dollar from './dollar.svg'


export const myAssets = {
    mail,
    dollar,
    right,
    bento1,
    bento2,
    bento3,
    bento4,
    bento5,
    blog1,
    blog3,
    blog2,
    blog4,
    blog5,
    blog6,
    blog7,
    blog8,
    coca_drink,
    ebi_wakame_udon_hot,
    beef_curry_udon,
    chicken_curry_udon,
    butadon_rice,
    com_bo_curry,
    com_ga_curry,
    com_heo_curry,
    com_tom_curry,
    egg_curry_udon,
    icon_account,
    gobouten_onsen_udon_hot,
    gobou_ebi_tempura_udon_hot,
    kitsune_niku_udon_hot,
    kitsune_onsen_udon_hot,
    liptonchanh_drink,
    logo,
    logo40x40,
    logo40x40_circle,
    niku_kamatama_udon_hot,
    niku_onsen_udon_hot,
    niku_wakame_udon_hot,
    pepsi_drink,
    poster3,
    poster4,
    poster5,
    poster6,
    poster7,
    promotion6,
    salad_cahoi,
    salad_dauhutrung,
    salad_rau,
    salad_rongtrung,
    salad_thittrung,
    salad_thitxongkhoi,
    salad_tom,
    samlanh_drink,
    sapporo_drink,
    sausage_curry_udon,
    shifudo_kimchi_udon_hot,
    shifudo_udon_hot,
    shishirian_raisu_rice,
    shrimp_curry_udon,
    sodablueocean_drink,
    sodachanh_drink,
    sodadao_drink,
    sodadau_drink,
    sodaruoumo_drink,
    sodavietquat_drink,
    tiger_drink,
    tradao_drink,
    trada_drink,
    trahoacuc_drink,
    unagi_udon_hot,
    yaki_kimchi_udon_hot,
    search,
    basket,
    menu,
    user,
    user1,
    user2,
    user3,
    user4,
    menu_close,
    badge,
    order_icon,
    cart_add,
    cart_added,
    cart_remove,
    dashboard,
    delivery,
    phone,
    forward,
    graph,
    heart_add,
    list,
    map,
    minus,
    plus,
    pound,
    return_request,
    secure,
    square_plus,
    trash,
    twitter,
    bg,
    star,
    feature1,
    feature2,
    feature3,
    star_black,
    
};
   
export const menuList=[
    {
        name:"Salad",
        img: salad_rau
    },
    {
        name:"Udon",
        img: kitsune_niku_udon_hot
    },
    {
        name:"Udon Curry",
        img: shrimp_curry_udon
    },
    {
        name:"Bento",
        img: bento1
    },
    {
        name:"Rice",
        img: com_bo_curry
    },
    {
        name:"Drink",
        img: coca_drink
    },

]

export const dummyProducts=[
    {
        id:"1",
        title:"Beef Curry Udon",
        img:[beef_curry_udon],
        price:{M:109000, L:159000},
        sizes:["M","L"],
        category: "Curry",
        inStock:true,
        popular:true,
    },
    {
        id:"2",
        title:"Beef Curry Udon",
        img:[beef_curry_udon],
        price:{M:109000, L:159000},
        sizes:["M","L"],
        category: "Drink",
        inStock:true,
        popular:true,
    },
    {
        id:"3",
        title:"Beef Curry Udon",
        img:[beef_curry_udon],
        price:{M:109000, L:159000},
        sizes:["M","L"],
        category: "Bento",
        inStock:true,
        popular:true,
    },
    {
        id:"4",
        title:"Beef Curry Udon",
        img:[beef_curry_udon],
        price:{M:109000, L:159000},
        sizes:["M","L"],
        category: "Udon",
        inStock:true,
        popular:true,
    },
    {
        id:"5",
        title:"Beef Curry Udon",
        img:[beef_curry_udon],
        price:{M:109000, L:159000},
        sizes:["M","L"],
        category: "Rice",
        inStock:true,
        popular:true,
    }
]

export const promotion=[
    {
        id:"1",
        img:[poster6],
        inStock:true,
    },
    {
        id:"2",
        img:[poster7],
        inStock:true,
    },
    {
        id:"3",
        img:[poster3],
        inStock:true,
    },
    {
        id:"4",
        img:[poster4],
        inStock:true,
    },
    {
        id:"5",
        img:[poster5],
        inStock:true,
    },
    {
        id:"6",
        img:[promotion6],
        inStock:true,
    },
];

// Blogs Dummy Data 
export const blogs = [
  {
    title: "Top 10 Skincare Must-Haves for 2025",
    category: "Skincare Tips",
    image: blog1,
    description:
      "Discover the essential skincare products you need in your routine for glowing, healthy skin in 2025.",
  },
  {
    title: "How to Choose the Perfect Foundation Shade",
    category: "Makeup Guide",
    image: blog2,
    description:
      "A step-by-step guide to finding your exact foundation match for a flawless and natural look.",
  },
  {
    title: "Hair Care Trends Every Woman Should Try",
    category: "Hair Care",
    image: blog3,
    description:
      "Explore the latest hair care tips and product trends that help strengthen, style, and shine.",
  },
  {
    title: "Beginner’s Guide to Building a Makeup Kit",
    category: "Beauty Basics",
    image: blog4,
    description:
      "A complete checklist of makeup essentials every beginner should own for daily and special looks.",
  },
  {
    title: "Maximizing Results with Your Night Skincare Routine",
    category: "Skincare Routine",
    image: blog5,
    description:
      "Learn how to create the perfect nighttime skincare routine for better hydration and anti-aging benefits.",
  },
  {
    title: "The Benefits of Organic and Natural Cosmetics",
    category: "Lifestyle",
    image: blog6,
    description:
      "Understand why more people are switching to natural cosmetics and the long-term skin benefits.",
  },
  {
    title: "Tips to Make Your Lipstick Last All Day",
    category: "Makeup Tips",
    image: blog7,
    description:
      "Simple but effective hacks to keep your lipstick fresh and vibrant from morning to evening.",
  },
  {
    title: "2025 Beauty Forecast: Makeup and Skincare Trends",
    category: "Beauty Trends",
    image: blog8,
    description:
      "An expert look at the upcoming beauty and skincare trends that will define style in 2025.",
  },
];

export const dummyAddress = [
  {
    _id: "67b5b9e54ea97f71bbc196a0",
    userId: "68591d36daf423db94fa8f4f",
    firstName: "user",
    lastName: "one",
    email: "userone@gmail.com",
    street: "789 Elm Street",
    city: "Springfield",
    state: "California",
    zipcode: 90210,
    country: "US",
    phone: "+1-555-123-4567",
  },
  {
     _id: "67b5b9e54ea97fdfgdbcsd5",
    userId: "68591d36daf423db94fa8f4f",
    firstName: "Jane",
    lastName: "Smith",
    email: "janesmith@gmail.com",
    street: "456 Elm Street",
    city: "Metropolis",
    state: "CA",
    zipcode: "998877",
    country: "United States",
    phone: "9876543210",
  },

];


export const dummyOrdersData = [
  {
    _id: "1",
    userId: "1",
    items: [
      {
        product: dummyProducts[0], // Argan Hair Oil
        quantity: 1,
        size: "50ml",
        _id: "1",
      },
      {
        product: dummyProducts[3], // Tea Tree Hair Oil
        quantity: 2,
        size: "100ml",
        _id: "2",
      },
    ],
    amount: 40.6,
    address: dummyAddress[0],
    status: "Out for delivery",
    paymentMethod: "COD",
    isPaid: false,
    createdAt: "2025-06-24T08:03:11.197+00:00",
    updatedAt: "2025-06-24T11:02:04.631+00:00",
    __v: 0,
  },
  {
    _id: "2",
    userId: "1",
    items: [
      {
        product: dummyProducts[8], // Vitamin C Face Oil
        quantity: 1,
        size: "30ml",
        _id: "1",
      },
      {
        product: dummyProducts[24], // Volumizing Shampoo
        quantity: 3,
        size: "400ml",
        _id: "2",
      },
    ],
    amount: 85.0,
    address: dummyAddress[0],
    status: "Delivered",
    paymentMethod: "Online",
    isPaid: true,
    createdAt: "2025-07-01T09:15:45.197+00:00",
    updatedAt: "2025-07-01T11:30:04.631+00:00",
    __v: 0,
  },
];

export const dashboardData = {
    "totalOrders": 125,
    "totalRevenue": 45230.75,
    // "orders": dummyOrdersData
}

